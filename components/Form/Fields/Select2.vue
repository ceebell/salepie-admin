<template>
  <div class="w-full">
    ...
    <div
      class="relative w-full font-mitr"
      :class="error ? 'border border-red-500 rounded-md' : ''"
      @mouseleave="clearHSSelectInputValue"
    >
      <select
        ref="selectEl"
        multiple
...
function clearHSSelectInputValue() {
  nextTick(() => {
...
function validate() {
  error.value = ''
  if (props.field.required && (!internalValue.value || internalValue.value.length === 0)) {
    error.value = 'จำเป็นต้องเลือกข้อมูล'
    return false
  }
  return true
}
...
const selectConfig = JSON.stringify({
  ...
  tagsInputClasses: "py-2 sm:py-1.5 px-2 grow order-1 sm:text-sm bg-transparent border-transparent rounded-md focus:outline-none focus:ring-0 dark:text-neutral-400 dark:placeholder:text-neutral-400",
